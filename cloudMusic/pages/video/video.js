...
Page({
   ...
   */
  data: {
    viderGroupList: [], //导航的标签数据
    navId: "", //导航的标识
    videoList: [], //视频的列表数据
    videoId: "",
    videoTime: [], //记录video播放的时长
  ...
  async getVideoGroupListData(){
    ...
    this.setData({
      viderGroupList: videoGroupListData.data.slice(0, 18),
      navId: videoGroupListData.data[0].id  //设置索引第一位为起始标记下划线 异步请求 非阻塞
    })
    //获取视频列表数据 navId是本地data里的数据
    this.getVideoList(this.data.navId); //发送请求同时传参
  },

  //请求视频列表数据
  async getVideoList(navId){ //异步请求 非阻塞
    if(!navId){ //判断navId为空串的情况
      return;
    }
    let videoListData = await request('/video/group',{id: navId}); //视频请求完毕
    //关闭消息提示框
    wx.hideLoading();
    let index = 0;
  ...
  //点击切换导航标识
  changeNav(event){
    let navId = event.currentTarget.id; //id是标签绑定获取到对象里的id 通过id向event传参的时候如果传的是number会自动转换为string
    //let navId = event.currentTarget.dataset.id; //通过data-id传入的number对象不会转换为string
    this.setData({
  ...
  handlePlay(event){
    ...
    * */
    ...
    //关闭上一个播放的视频 (这里在实现图片替代video时与点击图片自动播放中已经不存在多个视频同时播放)
    this.vid !== vid && this.videoContext && this.videoContext.stop(); //点击第二个视频，id不同于上次点击的id时，传入上一个视频的参数，触发暂停播放
    this.vid = vid; //上下文
    ...
    setTimeout(() => {
      //创建控制video标签的对象
      this.videoContext = wx.createVideoContext(vid); //第一次点击第一个视频，无参数传入
  ...
  bindTimeUpdate(event){
    ...
    let {videoTime} = this.data;
    /* 
      思路：判断记录播放时长的videoTime数组是否有当前视频播放的记录
        1.如果有，在原有的播放记录中修改播放时间为当前的时间
        2.如果没有，需要在数组中添加当前视频的播放对象
    */
   let videoItem = videoTime.find(item => item.vid === videoTimeObj.vid)
   if(videoItem){
  ...
  //视频播放结束时调用
  bindleEnded(event){
    //播放结束 移除记录播放时长数组中当前视频的对象
    let {videoTime} = this.data;
    let videoItemIndex = videoTime.findIndex(item => item.vid ===event.currentTarget.id)
    videoTime.splice(videoItemIndex,1)
    this.setData({
      videoTime: videoTime 
    })
  },

  //自定义视频下拉刷新的回调：scroll-view
  handleFresh(){
    //再次发请求，获取最新的视频列表数据
    this.getVideoList(this.data.navId); //传入最新的导航id
  },
